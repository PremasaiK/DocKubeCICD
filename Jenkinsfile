		...
		stage('Deploy to K8s')
		{
			steps{
				     sshagent(['premasaik-k8s'])
				      {
					sh 'scp -v -r -o StrictHostKeyChecking=no /home/premasai/DocKubeCICD/deployment.yaml /home/premasai/DocKubeCICD/service.yaml premasai@127.0.0.1:/home/premasai'
					...
					script{
						try{
							...
							println "${ret2}"	
						        if (ret1 == "Running") {
								      echo "execute portforward command on cosole: kubectl port-forward  ${ret0} 30010:3000"        
	     							}
							     else {
								      println "${ret1}"
							         }
